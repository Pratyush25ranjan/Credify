  ...
  return (
    ...
    >
      ...
      <Card className="overflow-hidden border-0 shadow-xl shadow-black/5">
        <div className="bg-gradient-to-r from-primary/5 via-primary/3 to-transparent p-8">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div>
              <p className="text-sm text-muted-foreground mb-2 font-medium">Verification Result</p>
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          { icon: Shield, label: 'Trusted Matches', value: result.trusted_matches, color: 'text-emerald-500' },
          { icon: FileText, label: 'Sources Checked', value: result.total_sources_checked, color: 'text-primary' },
          { icon: Brain, label: 'AI Confidence', value: `${result.confidence_score}%`, color: 'text-accent' },
        ].map((stat) => (
      ...
      {(trustedSources.length > 0 || otherSources.length > 0) && (
        <Card className="border-0 shadow-md shadow-black/5">
          <CardContent className="p-6">
            <h3 className="font-display font-semibold text-foreground mb-4">Sources Found</h3>
            <div className="space-y-3">
              {trustedSources.map((s, i) => <SourceCard key={`t-${i}`} source={s} />)}
              {otherSources.map((s, i) => <SourceCard key={`o-${i}`} source={s} />)}
            </div>
          </CardContent>
        </Card>
      )}
